...
*A geothermal reservoir-to-economics case study developed during the
Erasmus+ BIP Summer School "Geothermal Energy: Resource Assessment, Plants,
and Environmental Impacts," Politecnico di Torino (2026).*
...
The goal is to make every step from raw reservoir data to a final LCOE
number auditable and reproducible, with every simplifying assumption stated
...
The full directory tree — every script, figure, workbook, and output file,
with a one-line description of each — is maintained separately in
[`docs/REPOSITORY_STRUCTURE.md`](docs/REPOSITORY_STRUCTURE.md), including
which parts of `outputs/` are tracked in version control versus regenerated
by the pipeline.
...
<p align="center">
  <img src="figures/F9_spider_diagram.png"
       width="850"
       alt="Spider diagram of LCOE sensitivity to production temperature, reinjection temperature, flow rate, well depth, and drilling cost, each varied ±10%/±20% around the base case">
</p>
...
Figure F9 — plotted directly from the `Sensitivity` sheet's data in
`economics/Economic_Assessment_Group3.xlsx` (no values recalculated or
adjusted for the plot). It varies `T_source-in`, `T_source-out`, flow rate,
well depth, and drilling cost per metre by ±10%/±20% around the base case.
All five lines cross at the common base-case LCOE (186.5 €/MWh) by
construction — the base-case cell in each column references the workbook's
own `LCOE_Group3` sheet directly — which is also the expected behaviour of a
correctly constructed spider chart and is treated as a validation check (see
[Validation](#validation)). Drilling cost per metre and production
temperature are the parameters with the largest influence on LCOE for this
plant configuration. The well-depth and drilling-cost lines are numerically
identical at every point (shown here as solid vs. dashed so both remain
visible): in this formulation, CAPEX depends only on the *product* of well
depth and drilling cost per metre, so scaling either input by the same
percentage produces an identical LCOE.
...
- **Figures** — `figures/F1_temperature_maps.png` through
  ...
  `scripts/11_postprocess.py`; `figures/F9_spider_diagram.png` is plotted
  directly from the `Sensitivity` sheet's data in
